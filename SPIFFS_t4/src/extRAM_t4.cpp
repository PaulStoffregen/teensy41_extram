...
void extRAM_t4::flexspi_ip_write(uint32_t index, uint32_t addr, const void *data, uint32_t length)
{
  const uint8_t *src;
  uint32_t n, wrlen;
...
/*
   ...
   Timeout is optional
*/
bool extRAM_t4::waitFlash(uint32_t timeout) {
  uint8_t val;
  uint32_t t = millis();
...
static const u32_t blocksize = 4096; //or 32k or 64k (set correct flash commands above)
 ...
 s32_t extRAM_t4::spiffs_read(u32_t addr, u32_t size, u8_t * dst) {
  uint8_t *p;
  p = (uint8_t *)extBase;
  p += addr;
#ifdef FLASH_MEMMAP
 ...
 s32_t extRAM_t4::spiffs_write(u32_t addr, u32_t size, u8_t * src) {
  if(_spiffs_region == 0) {
 ...
 s32_t extRAM_t4::fs_erase(u32_t addr, u32_t size) {
  int s = size;
